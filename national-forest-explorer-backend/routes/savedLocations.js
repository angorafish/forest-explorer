...
router.get('/user/:userId', async (req, res) => {
  ...
  try {
    ...
    const detailedSavedLocations = await Promise.all(savedLocations.map(async (location) => {
      let details;
...
router.post('/save', async (req, res) => {
  ...
  try {
    const savedLocation = await SavedLocation.create({ userId, locationId, locationType });
