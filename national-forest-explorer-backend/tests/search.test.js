...
describe('Search Routes', () => {
    ...
    describe('GET /suggestions', () => {
        it('should fetch search suggestions', async () => {
            ...
            const response = await request(app).get('/search/suggestions').query({ q: 'test' });
        ...
        it('should return 500 if fetching suggestions fails', async () => {
            Campsite.findAll.mockRejectedValue(new Error('Failed to fetch suggestions'));
    ...
    describe('GET /', () => {
        it('should fetch search results', async () => {
            const mockCampsites = [{ id: 1, name: 'Campsite1', state: 'CA', forest: 'Forest1' }];
            const mockForests = [{ id: 1, name: 'Forest1', state: 'CA' }];
            const mockTrails = [{ id: 1, name: 'Trail1', state: 'CA', forest: 'Forest1' }];
            Campsite.findAll.mockResolvedValue(mockCampsites);
            Forest.findAll.mockResolvedValue(mockForests);
            Trail.findAll.mockResolvedValue(mockTrails);
            ...
            expect(response.body).toEqual([
                { id: 1, name: 'Campsite1', type: 'Campsite', state: 'CA', forest: 'Forest1' },
                { id: 1, name: 'Forest1', type: 'Forest', state: 'CA', forest: 'Forest1' },
                { id: 1, name: 'Trail1', type: 'Trail', state: 'CA', forest: 'Forest1' }
            ]);
        });

        it('should return 500 if fetching search results fails', async () => {
            Campsite.findAll.mockRejectedValue(new Error('Failed to fetch search results'));
