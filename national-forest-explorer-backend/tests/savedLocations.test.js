...
describe('Saved Locations Routes', () => {
    ...
    describe('GET /user/:userId', () => {
        it('should fetch saved locations for a user', async () => {
            const mockSavedLocations = [{ id: 1, locationId: 1, locationType: 'forest' }];
            ...
            expect(response.body).toEqual([{ ...mockSavedLocations[0], name: mockForest.name }]);
            expect(SavedLocation.findAll).toHaveBeenCalledWith({ where: { userId: '1' } });
        });

        it('should return 500 if fetching saved locations fails', async () => {
            SavedLocation.findAll.mockRejectedValue(new Error('Failed to fetch saved locations'));
    ...
    describe('POST /save', () => {
        it('should save a new location', async () => {
            const mockSavedLocation = { id: 1, userId: 1, locationId: 1, locationType: 'forest' };
            ...
            expect(SavedLocation.create).toHaveBeenCalledWith(expect.objectContaining({ userId: 1, locationId: 1, locationType: 'forest' }));
        });
