...
describe('User Routes', () => {
    ...
    describe('GET /username/:username', () => {
        it('should fetch user profile by username', async () => {
            const mockUser = { id: 1, username: 'testuser', sentRequests: [], receivedRequests: [], posts: [] };
            User.findOne.mockResolvedValue(mockUser);

            const response = await request(app).get('/user/username/testuser').set('Authorization', 'Bearer token');
    ...
    describe('GET /profile/:username', () => {
        it('should fetch user profile with posts by username', async () => {
            const mockUser = { id: 1, username: 'testuser', posts: [], sentRequests: [], receivedRequests: [] };
            User.findOne.mockResolvedValue(mockUser);

            const response = await request(app).get('/user/profile/testuser').set('Authorization', 'Bearer token');
    ...
    describe('POST /register', () => {
        it('should register a new user', async () => {
            ...
            expect(response.body).toEqual(mockUser);
            expect(User.create).toHaveBeenCalledWith(expect.objectContaining({ username: 'testuser', email: 'test@example.com' }));
        });
