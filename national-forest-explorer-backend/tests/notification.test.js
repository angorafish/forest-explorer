...
describe('Notification routes', () => {
    ...
    describe('GET /', () => {
        ...
        it('should return 500 if fetching notifications fails', async () => {
            ...
            Notification.findAll.mockRejectedValue(new Error('Failed to fetch notifications'));
    ...
    describe('PUT /:id/read', () => {
        it('should mark a notification as read for the authenticated user', async () => {
            ...
            const mockNotification = { id: 1, status: 'unread', save: jest.fn().mockResolvedValue(true) };
            Notification.findByPk.mockResolvedValue(mockNotification);

            const response = await request(app).put('/1/read').set('Authorization', 'Bearer token');
        ...
        it('should return 500 if marking the notification as read fails', async () => {
            ...
            Notification.findByPk.mockRejectedValue(new Error('Failed to mark notification as read'));

            const response = await request(app).put('/1/read').set('Authorization', 'Bearer token');

            expect(response.status).toBe(500);
            expect(response.body).toEqual({ error: 'Failed to mark notification as read' });
        });
    });
});